<template>
	<view id="mission">
		<view id="nav">
			...
			<view :class="(type=='noStart'?'nav_list on_nav_list':'nav_list')" @click="checkType('noStart')">未进行</view>
	...
	export default {
		...
		methods:{
			getTaskProcess(page){//正在进行中的任务
				util.getRequest(URL.TASK_PROCESS,{page:page,uid:this.userInfo.id},(results)=>{
			...
			getNoStart(page){//未开始的任务
				util.getRequest(URL.TASK_NO_START,{page:page,uid:this.userInfo.id},(results)=>{
						var results=results.data
			...
			getFinish(page){//已完成的任务
				util.getRequest(URL.TASK_FINISH,{page:page,uid:this.userInfo.id},(results)=>{
						var results=results.data
			...
			getProblemList(page){//问题清单的任务
				util.getRequest(URL.PROBLEM_LIST,{page:page,uid:this.userInfo.id},(results)=>{
						var results=results.data
